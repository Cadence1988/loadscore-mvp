...
export default function ShareResult({ form, result, targets, reloadScoreSource }) {
  const [status, setStatus] = useState("");
  const minimumRate = calculateMinimumRate({ ...form, ...targets }).minimumRate;
  const shareText = useMemo(
    () => buildShareText({ form, result, reloadScoreSource, minimumRate }),
    [form, minimumRate, reloadScoreSource, result],
  );
