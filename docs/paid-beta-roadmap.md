...
Status: **PRO-0 architecture only. PRO-1 has not begun.**
...
| PRO-1 Website Auth | PRO-0 approved; Supabase project/redirects available | Magic-link auth, callback, session provider, account shell, logout; no billing | Login/logout/expiry/account-switch tests; Free calculator unaffected | Create Supabase project, SMTP/from-address decision, approve domains | Yes, behind account entry points | No |
...
- Do not begin PRO-1 automatically from PRO-0.
...
## Exact next phase if approved

PRO-1 is limited to website magic-link authentication and an account shell:
